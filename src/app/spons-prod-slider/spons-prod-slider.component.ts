...
@Component({
	selector: 'app-spons-prod-slider',
	templateUrl: './spons-prod-slider.component.html',
	styleUrls: ['./spons-prod-slider.component.scss']
})
export class SponsProdSliderComponent implements OnInit, OnDestroy, AfterViewInit {
	...
	slides;
	slideslen;
	@ViewChild('sponsCont') slidecontainer: ElementRef;
	imageContainer;
	h3;
	sellPrice;
	genPrice;
	...
	initiateSlide(): void {
		...
		// this.slidecontainer = document.getElementsByClassName("sponsProdItemCont")['0'];
		...
		function resize() {
			let slidecontainer = me.window.document.getElementsByClassName('sponsProdItemCont')["0"];
	...
	dragStart(e): void {
		...
		SponsProdSliderComponent.initialPos = e.type == "touchstart" ? e.touches[0].clientX : e.x;
	...
	initiateDrag(me = this): void {
		me.slidecontainer = me.window.document.getElementsByClassName('sponsProdItemCont')['0'];
		me.slides = me.slidecontainer.nativeElement.getElementsByClassName('prodItem');
		let self = SponsProdSliderComponent;
		self.drag(event);
	}
